...
IF OBJECT_ID(N'fGetHotspotsE_vld', N'TF') IS NOT NULL
    DROP Function dbo.fGetHotspotsE_vld
GO
--
CREATE FUNCTION dbo.fGetHotspotsE_vld(@tlo float, @thi float)
--------------------------------------------------------------------
...
AS BEGIN
	...
	declare @t1 table(
		...
		area float not null)
	insert into @t1
	select f.sampleid, f.hpfid, primarea-- * annofrac area -- select f.sampleid, f.hpfid, primarea area 
	from test.dbo.RTFrac_vld f 
	where f.tumorfrac between @tlo and @thi
	and f.annofrac >= .25
	group by f.sampleid, f.hpfid, primarea --* annofrac-- group by f.sampleid, f.hpfid, primarea --
	-----------------------------------------------------------
...
--select * from test.dbo.DSamples order by samplei
*/